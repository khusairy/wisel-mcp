...
server.tool("create_story_draft", "Create a Wisel story as draft", { story: z.record(z.unknown()) }, async ({ story }) => result(await wisel.create(story)));
server.tool("update_story", "Apply a small editorial update", { id: z.string(), patch: z.record(z.unknown()) }, async ({ id, patch }) => result(await wisel.update(id, patch)));
